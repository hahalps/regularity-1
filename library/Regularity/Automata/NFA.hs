...
data NFA =
  NFA { states :: !(Set StateId)
            , delta :: !(Map StateId (Map Char (Set StateId)))
...
transitionsFor :: NFA -> StateId -> Map Char (Set StateId)
...
step :: NFA -> Set StateId -> Char -> Set StateId
...
char c =
  ...
  in
    NFA { states = Set.fromList [id0, id1]
              , delta = Map.singleton id0
                          (Map.singleton c (Set.singleton id1))
              , startState = id0
...
alt !a1 !a2 =
  ...
  in
    NFA { states = Set.insert id0 (states a1' `Set.union` states a2')
              , delta = Map.insert id0 init (delta a1' `Map.union` delta a2')
              , startState = id0
              , accepting =
                  Set.unions
